 ...
 * 
 * [추가 반영]
 *  - randomSeed(0)로 항상 동일한 벽 배치
 *  - autoMove=true 옵션: 일정 패턴으로 로봇 이동
 *  - testDuration=30초 후 CSV export & reset
 *  - [중요] 해상도/알고리즘 순차 실행 (experimentList) 로직 추가
 * 
 * [추가 수정]
 *  - 벽 배치: 테두리(전체 캔버스) + 내부에 3개의 랜덤 벽, 공간 여유 확보
 *  - 로봇 충돌 시 즉시 회피(벽 앞에서 오래 멈추지 않도록, 회피 타이머로 방향 전환)
 *************************************************************/
 ...
 ***************************************************/
let autoMove = true;           
let testDuration = 80000;  // 각각을 1분 30초 동안 실험.    
let startTime = 0;             
let movePhase = 0;             
let phaseTime = 0;             
...
class Robot {
  ...
  update(deltaTimeMS) {
    ...
    // 충돌 회피 중이라면, 일정 시간 동안 회전만
    if (this.wallAvoidTimer > 0) {
      this.heading += 0.03; // 계속 회전
      this.wallAvoidTimer -= 1;
      // 회피 중에는 전진/후진 X
      // 레이만 갱신
      this.updateRays();
      return;
